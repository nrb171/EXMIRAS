...
%   than alternatives like PCOLOR(), which uses surfaces instead of images.
    ...
    for ii = 1:numel(indsi)-1
        for jj = 1:numel(indsj)-1
            ...
            xlon = xlon2(...
                max(indsi(ii)-2,1):min(indsi(ii+1)+2, size(xlon2,1)), ...
                max(indsj(jj)-2,1):min(indsj(jj+1)+2, size(xlon2,2)));
            var = var2(...
                max(indsi(ii)-2,1):min(indsi(ii+1)+2, size(var2,1)), ...
                max(indsj(jj)-2,1):min(indsj(jj+1)+2, size(var2,2)));
